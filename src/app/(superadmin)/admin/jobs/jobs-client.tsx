  ...
  function run() {
    ...
    start(async () => {
      ...
      if (res?.data && "ok" in res.data && res.data.ok) {
        ...
        setMsg(`Done — ${s.remindersSent} reminder(s), ${s.approachingAlerts} approaching alert(s), ${s.transfers} transfer(s), ${s.staleNudges} nudge(s), ${s.followUpsDue} follow-up(s), ${s.ageingEscalations} ageing escalation(s).`);
      }
      router.refresh();
    });
  }
